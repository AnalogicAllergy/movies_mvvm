...
import com.woken.moviesmvvm.ui.single_movie_details.SingleMovie
...
class MovieItemViewHolder(view: View):RecyclerView.ViewHolder(view){
    fun bind(movie:Movie?, context: Context){
        ...
        itemView.cv_tv_movie_release_date.text = movie?.releaseDate!!.split("-")[0]
...
class NetworkStateItemViewHolder(view: View):RecyclerView.ViewHolder(view){
    fun bind(network: NetworkState){
       if(network != null && network == NetworkState.LOADING){
           itemView.pb_network_state.visibility = View.VISIBLE
       }else{
           itemView.pb_progress.visibility = View.GONE
       }
        if(network != null && network == NetworkState.ERROR){
            itemView.pb_network_state.visibility = View.VISIBLE
            itemView.tv_error_msg_item.text = network.msg

        }else if(network != null && network == NetworkState.ENDOFLIST){
            itemView.pb_network_state.visibility = View.VISIBLE
            itemView.tv_error_msg_item.text = network.msg
        }else{
            itemView.tv_error_msg_item.visibility = View.GONE
        }
    }
}